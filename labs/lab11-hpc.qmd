...
SLURM node:     squid01
CPUs allocated: 1
Yes, it is different since it uses 24 cores.
...
|--------|----------------|----------------|------------------|---------|
| `flu_1core.sh` | 1 | 3.01 | 2.60 | 1.16x |
| `flu_4core.sh` | 4 | 3.22 | 0.92 | 3.51x |

*Is the speedup on the cluster similar to what you observed on your laptop in Lab 10? What factors might cause differences?*

The speedup on the cluster is roughly similar in trend to what I saw on my laptop in lab 10, but not exactly the same. 
Factors might be:
- Different CPU hardware: the cluster node may have different clock speeds, cache sizes, and architecture than my laptop.
- Process startup and communication overhead: multiprocessing is not free, so the speedup is usually less than perfect 4×.
- Problem size: the simulation workload is not large enough, overhead takes a bigger fraction of runtime, reducing speedup.
---
...
- Deaths decrease as vaccination increases, with a slow decline up to 50%, followed by a sharp drop between 50% and 75%, indicating a herd immunity threshold. The most significant decrease occurs between 50% and 75% vaccination.
---
...
- The plots show a clear herd immunity threshold around 75% vaccination. Below this level, infections and deaths decrease only gradually, but once vaccination exceeds this threshold, disease spread collapses and deaths drop sharply, indicating that transmission is no longer sustainable.
---
...
*Paste the output.*
- Done in 0.28s, sum=333332833333500000
...
1. SLURM is the job scheduler that manages resources and runs jobs fairly across users. You can’t SSH to compute nodes because that would bypass SLURM and disrupt resource allocation for others.

2. A SLURM job array lets you run multiple similar jobs with one script, using different task IDs. It’s more convenient when only inputs or parameters change, avoiding many separate scripts.

3. The speedup is similar in general, but not identical. Differences come from CPU hardware, clock speed, memory bandwidth, and system load. Laptops may also have background processes or thermal throttling.

4. You ran 500 simulations total. Serial time ≈ 5 × (time for 100 simulations). With the job array, tasks run in parallel, so total time ≈ time of one task, plus small scheduling overhead.