...
- The top 5 medical specialties are Surgery, Consult-History and Phy, Cardiovascular/Pulmonary, and Orthopedics, Radiology. Surgery has the most that goes over 1000, while the rest has way less. Not evenly distributed. 
...
- The top 20 most frequent words are mostly punctuation and stopwords. The word patient appears very also has high frequency but it is not a very informative word in medical records. So we don't really see much insights.
...
- When we remove stopwords, we see some more informative words like pain, blood, and skin. But still mostly punctuation and stopwords. It gives us a little bit better idea of what the text is about.
...
# Add custom stopwords
additional_stopwords = {'procedure', 'also', 'right', 'left'}  # Hint: add 3-4 clinical filler words
...
- There are 806 clinical stopwords. I removed procedure, also, right, and left. The results differ  when using clinical stopwords vs. general stopwords. We see more informative words like pain, blood, and skin when using clinical stopwords. However there are still quite a few stopwords remaining.
...
- The top 20 most frequent bigrams are like operating room, oreooed draped or blood pressure. These make sense in medical records.
...
- The words before and after operating are mostly related to the operating room and the procedure. The most frequent word after operating is room, and the most frequent word before operating is taken.
...
(ggplot(mt_samples[mt_samples['num_sentences'] > 0], aes(x='avg_sentence_length'))  # Hint: column name
...
- The average sentence length is generally longer for Speech, office Notes and lab Medicine, shorter for Hospice, Chiropractic or Autopsy. Most of the average sentence length is around 20-22.5 words.
...
- It is clearer when there are 5 topics. We can see more distinct topics and the words are more informative.
...
- The topic Cardiovascular / Pulmonary went to the top, while Surgery went to the fifth place. The top 5 topics align with the top 5 medical specialties, but the order changed.
