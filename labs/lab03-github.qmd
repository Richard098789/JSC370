...
No missing data in `temp` and `elev`
...
- Maximum temperature: 999.9
- Minimum elevation: -35.0
- Maximum elevation: 3680.0
...
Summary:Removed temperatures greater than 60

```{python}
cold_temps = met[met['temp'] < 0]

# display data here extract lat and lon and check google
cold_temps[['lat', 'lon']].describe()
```

Summary:345 rows of data with temperatures less than 0, latitude range from 28 to 47, longitude range from -120 to -71
...
min_elev_station = met[met['elev'] == min_elev].copy()
...
```{python}
plt.hist(min_elev_station['temp'])
...
Summarize time series plot: The temperature gradually increases from 6-20 to 7-01, 2025
...
- The lowest elevation station is located at Indigo
